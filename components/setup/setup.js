...
//   OS Detection → Python & Tools → Python Dependency →
//   GPU Detection → System Compatibility → Framework Selection →
//   Framework Install → Framework Verification →
//   Resources → Project Folder → QoL → Security → Finish
//
  ...
  const STEPS = [
      ...
      afterRender: async () => {
        async function runPyDepScan() {
          ...
          try {
            ...
            if (tbody) {
              tbody.innerHTML = `
                <tr><td>${pythonVer ? '✅' : '❌'}</td><td>Python 3</td><td class="setup-hint">${escapeHtml(pythonVer || 'not found')}</td></tr>
            ...
            if (pythonVer) {
              out.innerHTML = `<div class="setup-success-msg">✅ Python ${escapeHtml(pythonVer)} detected</div>`;
            ...
            out.innerHTML = `<p class="setup-hint">⚠️ Check failed: ${escapeHtml(e.message)}</p>`;
          } finally {
            if (rescanW) rescanW.style.display = 'block';
          }
        }
   ...
   // ── 9: Framework Install ──────────────────────────────────────
   {
     ...
     render: () => {
      ...
      afterRender: async () => {
        ...
        document.getElementById('run-fw-install-btn')?.addEventListener('click', async () => {
          ...
          if (scopeSelect?.value === 'project' && !selectedProjectFolder) {
          ...
          try {
            ...
            const exitCode = await window.electron.runInstallStream(fw, gv, accelVersion, scope, selectedProjectFolder);
    ...
    // ── 12: Project Folder ─────────────────────────────────────────
    {
      ...
      afterRender: async () => {
        ...
        try {
          ...
          if (recents?.open) {
            ...
            if (current) current.innerHTML = `<div class="setup-success-msg">📂 Current: <strong>${escapeHtml(selectedFolder)}</strong></div>`;
          }
        } catch (_) {}
        ...
        document.getElementById('project-folder-pick-btn')?.addEventListener('click', async () => {
          try {
            ...
            const current = document.getElementById('project-folder-current');
            if (current) current.innerHTML = `<div class="setup-success-msg">📂 Selected: <strong>${escapeHtml(folderPath)}</strong></div>`;
            ...
            if (statusDiv && initDiv) {
              ...
              if (missing.length > 0) {
                statusDiv.style.display = 'block';
                statusDiv.innerHTML = `<p>Missing: ${missing.map(m => `<strong>${escapeHtml(m)}</strong>`).join(', ')}</p>`;
      ...
      collect: () => {
        // The folder path is saved to recents when user picks it via projectPickFolder + openFolder
